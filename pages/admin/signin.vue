<template>
    <div class="login-page" >
        ...
        <form class="ui form" @submit.prevent="userLogin" :class="{loading:loading}">
            <div class="header">
                 <img src="~/assets/images/logo.png" width="70px" alt="">
                 <h2 style="color:rgba(255,255,255,0.7)">Admin Sign In</h2>

                <div class="ui negative message" v-if="serverError">
                    <i class="close icon" @click="hideMes"></i>                                                                                                                                                                                                                                       <div class="header">
                           {{serverError}}
                        </div>
                        <p>Opps,something went Wrong please type in correct user name and password
                        </p>
                    </div>
                </div>
            <!-- <validation-provider rules="required" v-slot="{ errors }" > -->
                <div class="field" :class="{error:false}">
   ...
   export default {
       components: {
        ...
        methods:{
            ...
            async userLogin() {
                try {
                    let response = await this.$auth.loginWith('local', { data: this.newUser })
                } catch (err) {
